...
if check_password():
    # --- 2. GITHUB API FUNKTIONEN (Mit Retry-Logik) ---
    def safe_github_request(filename, method="GET", content=None, max_retries=3):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    st.error(f"Verbindungsfehler zu GitHub bei {filename}. Bitte Seite neu laden.")
                    return None, None
        return None, None
    ...
    if 'df' not in st.session_state:
        with st.spinner("Initialisiere Daten..."):
            ...
            if raw_cache:
                ...
                st.session_state.df = pd.DataFrame(columns=['title', 'link', 'source_name', 'category'])
    ...
    if not df.empty:
        for q, group in df.groupby("source_name"):
            exp_key = f"exp_{q}"
            is_expanded = st.session_state.expander_state.get(exp_key, False)
